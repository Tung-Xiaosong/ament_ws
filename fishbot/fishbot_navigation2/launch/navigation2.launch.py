...
def generate_launch_description():
    ...
    #dxs add
    map_yaml_dir = get_package_share_directory('robot_navigation2') 
    robot_model_dis = get_package_share_directory('robot_description')

    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
    # map_yaml_path = LaunchConfiguration('map',default=os.path.join(fishbot_navigation2_dir,'maps','fishbot_map.yaml'))
    map_yaml_path = LaunchConfiguration('map',default=os.path.join(map_yaml_dir,'map','g1.yaml'))#dxs change
    ...
    return LaunchDescription([
        ...
        #dxs add
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource([robot_model_dis, '/launch', '/load_robot_model_into_gazebo.launch.py'])
        ),
        Node(
            package='rviz2',
            executable='rviz2',
