...
options = {
  ...
  odom_frame = "odom",                      -- 里程计的坐标系名字(表示需要用cartographer发布的里程计坐标系的名字)
  provide_odom_frame = false,               -- 是否提供odom的tf, 如果为true,则tf树为map->odom->footprint(表示是否需要让cartographer提供一个里程计坐标系)
  ...
  num_multi_echo_laser_scans = 0,           -- 是否使用multi_echo_laser_scans数据(是否使用多回声波雷达)
...
TRAJECTORY_BUILDER_2D.max_range = 50.  --dxs change 6 激光雷达最大扫描距离
